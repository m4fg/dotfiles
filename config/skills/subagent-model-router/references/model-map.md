...
モデルの世代が変わったら、このファイルだけを直す。SKILL.md の規則は段で書かれているので触らなくてよい。日付が古い、あるいは指定したモデルが使えないという警告が出たら、末尾の出典で現在の構成を確かめてから割り当てる。
...
|---|---|---|
| **M** | Sonnet 5 — `sonnet` × `medium` | GPT-5.6 Terra — `gpt-5.6-terra` × `medium` |
| **H** | Opus 5 — `opus` × `high` | GPT-5.6 Sol — `gpt-5.6-sol` × `high` |
| **X** | Fable 5.1 — `fable` × `high` | GPT-6 Astra — `gpt-6-astra` × `high` |
| L(既定では使わない) | Haiku 4.5 — `haiku`(effort は指定できない) | GPT-5.6 Luna — `gpt-5.6-luna` × `low` |
...
- **OpenAI 系の Luna** は effort が使え、文脈も上位と同じ大きさ。二次情報では単価が Terra のおよそ 10 分の 1 なので、量の多い定型作業では下げる利得が大きい。
- L の失敗は、まず M に戻す。
...
- OpenAI 系(二次情報。改定が多いので出典で確かめる): Terra $2 / $12、Sol $5 / $30(期間限定の割引あり)、Astra $10 / $50

どちらの系統も、M → H が約 2.5 倍、H → X が約 2 倍。同じモデルでも effort を上げると消費トークンが数倍になることがあり、段を 1 つ上げるより高くつく場合がある。考えても結果が変わらない仕事は、モデルを下げるより effort を下げる方が効くことが多い。
...
- M(Sonnet 5 / Terra)が選べない → M の仕事は H × `low` で代用する。
- 引退予定に注意する。`gpt-5.5` は 2026-10-14 に ChatGPT サインインの Codex から外れる(後継は `gpt-5.6-sol`)。`gpt-5.4` と `gpt-5.4-mini` は既に外れている。ユーザーの既存の定義にこれらが残っていたら、置き換えを一言提案する。
...
- OpenAI の単価: https://learn.chatgpt.com/api/docs/pricing
